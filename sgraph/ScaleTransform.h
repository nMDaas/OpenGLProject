...
namespace sgraph {
    ...
    protected:
        ...
        ParentSGNode *copyNode() {
            return new ScaleTransform(sx,sy,sz,name,scenegraph,lights);
        }    

    public:
        ScaleTransform(float sx,float sy,float sz,const string& name,sgraph::IScenegraph *graph, vector<util::Light> i_lights) 
