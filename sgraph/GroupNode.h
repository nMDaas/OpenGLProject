 ...
 */
  ...
  protected:
    ParentSGNode *copyNode() {
      return new GroupNode(name,scenegraph,lights);
    }
    

  public:
    GroupNode(const string& name,sgraph::IScenegraph *graph,vector<util::Light> i_lights)
      :ParentSGNode(name,graph,i_lights) {      
    }
    ...
    SGNode *clone() {
      ...
      GroupNode *newgroup = new GroupNode(name,scenegraph,lights);
